...
INSERT_BEFORE = '        return `<div class='
AVG_BLOCK = (
    '        const firsts=staffRows.filter(p=>p.was!=null).map(p=>Number(p.was));\n'
...
def patch(text):
    ...
    if INSERT_BEFORE in text and 'Средний по отделу' not in text:
        text = text.replace(INSERT_BEFORE, AVG_BLOCK + INSERT_BEFORE, 1)
        print('avg calc')
